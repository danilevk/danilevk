		...
		nh_("~"), QWidget(parent)
{
	...
	Drone_Selec = new QLabel("Drone_Select", this);
	Speed = new QLabel("Pitch", this);
	Alt_speed = new QLabel("Roll", this);
	
	num_drone = new QLabel("", this);
	...
	font.setPointSize(20);
	font.setBold(true);
	num_drone->setFont(font);
	
	Drone_Selec->setText("Dron Select:");
    Speed->setText("Speed:");
    Alt_speed->setText("Altitude Speed:");
	
	grid = new QGridLayout(this);
	
	grid->addWidget(Drone_Selec, 0, 0);
	grid->addWidget(Speed, 1, 0);
	grid->addWidget(Alt_speed, 2, 0);
	
	grid->addWidget(num_drone, 0, 1);
...
void KeyPress::keyPressEvent(QKeyEvent *event) 
{
	...
	{	
		drone_selected = 1;
		num_drone->setText(QString::number(drone_selected));
		setLayout(grid);
		send_command = false;
	}
	...
	{	
		...
		Command->setText("Reseting Overrides");
